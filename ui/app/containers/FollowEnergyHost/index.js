...
const FollowEnergyAdmin = () => {
  ...
  const { roomId, name, id } = useParams();
  ...
  const [, setExportInfo] = useState({});
  const [exportRoomName, setExportRoomName] = useState('');
  const [, setExportTotalKwh] = useState('');
  const [, setExportLastKwh] = useState('');
  const [, setExportLastWater] = useState('');
  const [, setExportLatestKwh] = useState('');
  const [, setExportLatestWater] = useState('');
  ...
  const handleButtonFilter = async () => {
    ...
    if (endDateDisplay < startDateDisplay) {
      ...
      const firstDayLastMonth = moment(startDateDisplay)
        .subtract(1, 'months')
        .startOf('month')
        .format('YYYY-MM-DD');
      const lastDayLastMonth = moment(startDateDisplay)
        .subtract(1, 'months')
        .endOf('month')
        .format('YYYY-MM-DD');
      const apiGetLastMonth = `${urlLink.api.serverUrl +
        urlLink.api.getTotalKWhPerDayForDayToDayV2 +
        roomId}/${firstDayLastMonth}/${lastDayLastMonth}`;
  ...
  const handleExportPdf = async () => {
    try {
      ...
      pdfDoc.text(
        `Electric Unit Price: ${formattedElectricUnitPrice} (VND/kWh)`,
        30,
        60,
      );
  ...
  const getCurrentDayData = async () => {
    ...
    const apiUrlDay = `${urlLink.api.serverUrl +
      urlLink.api.getTotalKWhPerHourInOneDayV2 +
      roomId}/${moment().format('YYYY-MM-DD')}`;

    const apiUrlMon = `${urlLink.api.serverUrl +
      urlLink.api.getTotalKWhPerDayInOneMonthV2 +
      ...
      <>
        ...
        <Breadcrumb className="">
          ...
          <BreadcrumbItem>
            {!_.isEmpty(state) && (
              <NavLink
                to={`${urlLink.managerEnergyRoomsHost}/${id}/${
                  state.motelName
                }`}
              >
                {state.motelName}
                  ...
                  <span className="content">
                    ...
                    {info.kWhData && info.kWhData.length > 0
                      ? info.kWhData
                          ...
                          .reduce(
                            (tổng, giáTrị) => tổng + parseFloat(giáTrị),
                            0,
                          )
                          .toFixed(2)
                      : '0.00'}{' '}
                      ...
                      <tr key={index}>
                        ...
                        {info.kWhData ? (
                          info.kWhData[index] !== null ? (
                            <td>
                              {parseFloat(info.kWhData[index]).toFixed(2)}
                            </td>
                          ) : (
                        ...
                        <td>
                          {info.kWhData !== null && info.waterData !== undefined
                            ? info.kWhData[index] !== null &&
