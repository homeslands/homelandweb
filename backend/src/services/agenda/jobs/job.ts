        ...
        if (!jobData.isActived && !jobData.isDeleted) {
          if (userData) {
            ...
            await NotificationController.createNotification({
              ...
              content: `Phòng ${roomData.name} thuộc tòa nhà ${
                motelData.name
              } đặt 
    ...
    try {
      ...
      if (jobData) {
        ...
        if (moment().year() < checkOutTime.year()) {
          console.log(
            "RemindUserMonthly15EveryDay_ExpireNextMonth checkOutTime",
            checkOutTime
          );
          console.log(
            "RemindUserMonthly15EveryDay_ExpireNextMonth moment",
            moment()
          );
          //CHECK
          ...
          if (moment().month() < checkOutTime.month()) {
            //CHECKED
            console.log(
              "RemindUserMonthly15EveryDay_ExpireNextMonth 2 checkOutTime",
              checkOutTime
            );
            console.log(
              "RemindUserMonthly15EveryDay_ExpireNextMonth 2 moment",
              moment()
            );
      ...
      try {
        ...
        if (orderData) {
          ...
          if (orderData.isCompleted === false) {
            ...
            if (checkOutTimePlus3Days.diff(moment().endOf("day")) >= 0) {
              if (userData) {
                await NotificationController.createNotification({
                  ...
                  content: `Quý khách vui lòng đóng tiền phòng tháng${checkOutTime.month() +
                    1}/${checkOutTime.year()} 
                cho phòng ${jobData.room.name} thuộc dãy ${
                    jobData.motelRoom.name
                  }. Hạn đóng tới hết ngày 
                03/${moment().month() +
                  1}/${moment().year()}. Lưu ý: Nếu không hoàn thành thanh toán, 
              ...
              if (userData.email) {
                ...
                const mailOptions = {
                  ...
                  text: `Phòng ${roomDataN.name} thuộc tòa nhà ${
                    motelDataN.name
                  } của quý khách đã bị hủy hợp đồng vì không thanh toán hóa đơn tháng ${moment(
                    orderData.startTime
                  ).format(
                    "MM-YYYY"
                  )} đúng hạn. Quý khách sẽ không được hoàn các khoản cọc trước đó.`,
                };
              ...
              await NotificationController.createNotification({
                ...
                content: `Phòng ${roomDataN.name} thuộc tòa nhà ${
                  motelDataN.name
                } 
              ...
              if (userData) {
                await NotificationController.createNotification({
                  ...
                  content: `Hợp đồng cho thuê phòng ${
                    jobData.room.name
                  } thuộc dãy ${jobData.motelRoom.name} của 
            ...
            const userData = await userModel
              .findOne({ _id: jobDataAfterUpdate.user })
              .lean()
              .exec();
          ...
          if (orderData.isCompleted === false) {
            ...
            if (moment().date() <= 4) {
              if (userData) {
                ...
                04/${checkOutTime.month() +
                  1}/${checkOutTime.year()}. Lưu ý: Nếu không hoàn thành thanh toán, 
                quý khách sẽ không được hoàn trả tiền cọc.`,
                ...
                if (userData.email) {
                  ...
                  const mailOptions = {
                    ...
                    text: `Hợp đồng cho thuê phòng ${
                      ...
                      jobData.motelRoom.name
                    } của quý khách đã bị hủy vì quý khách chưa hoàn thành tiền phòng tháng ${moment(
                      orderData.startTime
                    ).format("MM-YYYY")} đúng hạn.`,
                  };
            ...
            if (moment().date() <= 6) {
              ...
              if (userData) {
                await NotificationController.createNotification({
                  ...
                  content: `Quý khách vui lòng đóng tiền phòng tháng ${checkOutDay.month() +
                    1}/${checkOutDay.year()} 
                cho phòng ${jobData.room.name} thuộc dãy ${
                    jobData.motelRoom.name
                  }. Hạn đóng tới hết ngày 
                06/${checkOutDay.month() +
                  1}/${checkOutDay.year()}. Lưu ý: Nếu không thực hiện đóng hóa đơn này, 
                quý khách sẽ không được hoàn trả tiền cọc.`,
              ...
              const roomDataN = await roomModel
                .findOne({ _id: jobDataAfterUpdate.room })
                .lean()
                .exec();
              const floorDataN = await floorModel
                .findOne({ rooms: jobDataAfterUpdate.room })
                .lean()
                .exec();
          ...
          if (orderData.isCompleted === false) {
            ...
            if (moment().date() <= 15) {
              ...
              if (userData) {
                ...
                ngày 15/${moment().month() +
                  1}/${moment().year()}. Lưu ý: Nếu không hoàn thành đúng hạn, 
            ...
            if (moment().date() <= checkOutDay.date()) {
              ...
              if (userData) {
                await NotificationController.createNotification({
                  ...
                  content: `Quý khách vui lòng đóng tiền phòng tháng ${timeCal.month() +
            ...
            if (checkOutDay.clone().diff(moment(), "months") < 1) {
              ...
              if (userData) {
                await NotificationController.createNotification({
                  ...
                  content: `Phòng ${resData.room.name} thuộc dãy ${
    ...
    async (job, done) => {
      try {
        ...
        for (let i = 0; i < cleanArray.length; i++) {
          ...
          await RoomController.updateInforMotel(floorDataN, motelRoomDataN);
...
async function createOrderHistory(
  ...
  const bankData = await BankingModel.findOne({ _id: transactionsData.banking })
    .lean()
    .exec();
